...
// Importar interfaces desde la nueva ubicación
...
export class DataSeedingService implements OnApplicationBootstrap {
  ...
  async seedDatabase() {
    ...
    try {
      argentinaCountry = await this.countriesService.findOneByName('Argentina');
      if (!argentinaCountry) {
        const countryDto: CreateCountryDto = { name: 'Argentina', code: 'AR' };
        argentinaCountry = await this.countriesService.create(countryDto);
      ...
      for (const georefProv of georefProvinces) {
        ...
        try {
            const provinceDto: CreateProvinceDto = {
                ...
                // georefId: georefProv.id, // Ya no usamos georefId en la entidad Province
            };
            const processedProvince = await this.provincesService.create(provinceDto);
            ...
            if (!(error instanceof ConflictException)) {
                ...
                const existingProv = await this.provincesService.findOneByNameAndCountryId(georefProv.nombre, argentinaCountry.id);
                if (existingProv) {
      ...
      for (const georefMuni of georefMunicipios) {
        ...
        try {
          const cityDto: CreateCityDto = {
            ...
            // georefId: georefMuni.id, // Ya no usamos georefId en la entidad City
          };
          const processedCity = await this.citiesService.create(cityDto);
          ...
          citiesProcessed++;
            ...
            if (!(error instanceof ConflictException)){
                ...
                citiesFailed++;
                 ...
                 const existingCity = await this.citiesService.findOneByNameAndProvinceId(georefMuni.nombre, provinceData.localId);
                 if(existingCity) {
